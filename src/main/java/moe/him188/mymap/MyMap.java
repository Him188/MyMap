...
import moe.him188.mymap.event.FrameAddEvent;
import moe.him188.mymap.event.FrameRemoveEvent;
import moe.him188.mymap.utils.FrameList;
import moe.him188.mymap.utils.LanguageBase;
...
public final class MyMap extends PluginBase {
    ...
    public void onEnable() {
        ...
        } else if ("auto".equalsIgnoreCase(language)) {
            LanguageBase.CURRENT_LANGUAGE = LanguageBase.getDefaultLanguage();
        } else {
    ...
    public void onDisable() {
        ...
        for (MyMapFrame frame : this.getList()) {
            for (Vector3 position : frame.getBlocks()) {
                frame.getLevel().setBlock(position, new BlockAir());
            }
        }
    }
